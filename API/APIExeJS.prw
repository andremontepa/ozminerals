...
//=============================================================================================================================
	...
	If .not. Empty(cBody)
			...
			For nX := 1 To Len(aPedidos)
				For nY := 1 To Len(aPedidos[nX])
					If Upper(Alltrim(aPedidos[nX][nY][1])) <> "PK"
						...
						EndIf
						If Alltrim(aPedidos[nX][nY][1]) $ "C7_FILIAL|C7_NUM|C7_EMISSAO|C7_FORNECE|C7_LOJA|C7_COND|C7_CONTATO|C7_FILENT|C7_VLADTO "
							If Alltrim(::cOper) == "POST"
								If Alltrim(aPedidos[nX][nY][1]) <> "C7_NUM"
									If nX == 1
										aadd(aCabec,{aPedidos[nX][nY][1],xValor,Nil})
										If Alltrim(aPedidos[nX][nY][1]) == "C7_FILIAL"
											aAdd(aAdto, {"FIE_FILIAL", xValor,                                     Nil})
										EndIf
										aAdd(aAdto, {"FIE_CART",   "P",                                            Nil}) // Carteira pagar
										aAdd(aAdto, {"FIE_PEDIDO", "" ,                                            Nil}) // Não precisa, pois quem trata é a MATA120
										aAdd(aAdto, {"FIE_PREFIX", PadR("PA", TamSX3("FIE_PREFIX")[1]),            Nil}) //Prefixo
										If Alltrim(aPedidos[nX][nY][1]) == "C7_NUM"
											aAdd(aAdto, {"FIE_NUM",    PadR("PA"+xValor, TamSX3("FIE_NUM")[1]),   Nil}) //Numero Titulo
										EndIf
										aAdd(aAdto, {"FIE_PARCEL", PadR("1", TamSX3("FIE_PARCEL")[1]),             Nil}) //Parcela
										aAdd(aAdto, {"FIE_TIPO",   PadR("PA", TamSX3("FIE_TIPO")[1]),              Nil}) //Tipo = PA
										If Alltrim(aPedidos[nX][nY][1]) == "C7_FORNECE"
											aAdd(aAdto, {"FIE_FORNEC", PadR(xValor, TamSX3("FIE_FORNEC")[1]),          Nil}) // Fornecedor
										EndIf
										If Alltrim(aPedidos[nX][nY][1]) == "C7_LOJA"
											aAdd(aAdto, {"FIE_LOJA",   PadR(xValor, TamSX3("FIE_LOJA")[1]),         Nil}) //Loja
										EndIf
										If Alltrim(aPedidos[nX][nY][1]) == "C7_VLADTO"
											aAdd(aAdto, {"FIE_VALOR",  xValor,                                          Nil}) // Valor do pa que está vinculado ao pedido
											nVlAdto :=  xValor
										EndIf
									EndIf
								Else
					...
					EndIf
				Next nY
				If nX == 1 .and. nVlAdto > 0
					aAdd(aAdtPC, aClone(aAdto))
				EndIf
		...
		Else
			...
			::SetRespError(500,cMsg)
			Return {aCabec,aItens,aDel,cFiltro,cMsg,aAdtPC}
		EndIf
	Else
		...
		If ::aTab[1] == "SC7"
			If Alltrim(::cOper) == "POST" .or. Alltrim(::cOper) == "PUT"
				If Len(aCabec) > 0 .and. Len(aItens) > 0
                    MSExecAuto({|a,b,c,d,e,,g| MATA120(a,b,c,d,e,,g)},1,aCabec,aItens,nOper,.F.,,aAdtPC) // 3 - Inclusao, 4 - Alteração, 5 - Exclusão
				Else
					If Len(aCabec) <= 0

...
//====================================================================================================
    ...
    nPos := At(";",cLinha)
    While !Empty(cLinha)  .And. nPos > 0
        aStrCpo := FWSX3Util():GetFieldStruct(Alltrim(cLinha))
        If Len(aStrCpo) > 0 .and. aStrCpo[1] == Alltrim(Upper(SubStr(cLinha,1,nPos-1)))
            Aadd(aCampos,{Alltrim(aStrCpo[1]),GetSX3Cache(aStrCpo[1],"X3_ORDEM"),Len(aCampos)+1,Alltrim(aStrCpo[2]),"SZ1"})
        Else
            cMens += Alltrim(SubStr(cLinha,1,nPos-1))+Chr(13)+Chr(10)
        EndIf
        cLinha := SubStr(cLinha,nPos+1,Len(cLinha)-nPos)
        nPos  := At(";",cLinha)
        If nPos = 0 .And. !Empty(cLinha)

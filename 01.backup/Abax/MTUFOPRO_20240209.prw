...
Programador		Data		
---------------------------------------------------------------------
Leonardo Viana          01/2024                 

Criada função NotaJaErp para verificar se a nota já está registrada no Protheus, caso esteja, não segue o processo e muda o status para três,
este desenvolvimento foi feito, para sanar um problema local do cliente OZ Minerals.
	...
	Do While ZNFTEMP->(!Eof())
		...
		If !lImport		                                 
         //Trocado Log para mostrar s o uma vez no console.log, da forma antiga a nota era grava para cada item da mesma.          //Leonardo Vasco Viana 21/07/2017
		   ...
		   If FieldPos("ZNF_CNPJEN") > 0  
				Aadd(_aCabSF1,{"F1_XCNPJEN"  ,ZNFTEMP->ZNF_CNPJEN			   		,Nil})  //Se a Nota Fiscal Tiver o campo seguro preenchido serÃ¡ enviado ao ERP. 	   			              
		   Endif
	   ...
	   dbSelectArea('SC7')  
		...
		If FieldPos("ZNF_ABATIN") > 0 
		 ...
		 If FieldPos("ZNF_BASEIN") > 0
			If ZNFTEMP->ZNF_BASEIN > 0
		 ...
		 //Informações do PCC inseridas no Preenchimento
		...
		If ZNFTEMP->ZNF_ALQCOF > 0	
			Aadd(_aLinha,{"D1_ALQCOF" ,ZNFTEMP->ZNF_ALQCOF	,Nil})  //326 a 342 Tratamento para Impostos de Serviços 
		Endif
...
User Function FSGeraCTE(cPrefix)
	...
	Do While ZNFCTE->(!Eof())
		...
		if u_NotaJaErp(cFilAnt, cDocSMA, cSerSMA, cForSMA, cLojSMA)	
			...
			lMsErroAuto := .T.
			lImport:= .T.
			cError += ' Erro: CTE está na ZNF mas foi registrada manualmente no Protheus. Este procedimento poderá comprometer o bom funcionamento do integrador.'
		Endif
				...
				IF Empty( __cLogMsg)  
				   If Len(aLog) > 0 
						...
						For nlog:=1 to Len(aLog)
							__cLogMsg += 'ERRO EXECAUTO ' + aLog[nlog] + ';'
						Next
...
**********************************************************************************************/
	...
	cQryExc += CRLF +" F1_DOC = '"+cDocNFe+" ' and "    
	cQryExc += CRLF +" F1_SERIE = '"+cSerNFe+" ' and "    
	cQryExc += CRLF +" F1_FORNECE = '"+cForNFe+" ' and "    
	cQryExc += CRLF +" F1_LOJA = '"+cLojNFe+" ' and "    
	cQryExc += CRLF +" D_E_L_E_T_ <> '*' "

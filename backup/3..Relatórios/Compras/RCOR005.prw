...
Local oReport := nil
Local cPerg:= PadR("RCOR005", Len(SX1->X1_GRUPO))
...
oSection2:= TRSection():New(oReport, "Classe", {"TMP"}, NIL, .F., .T.)
TRCell():new(oSection2, "ITEMCON"   ,"TMP","ITEM CONTABIL" ,,TAMSX3("ZW2_ITEMCO")[1]+1,/*lPixel*/,/*{|| code-block de impressao }*/)
TRCell():new(oSection2, "CONTA"	   ,"TMP","CLASSE" ,,TAMSX3("ZW2_CONTA")[1]+1,/*lPixel*/,/*{|| code-block de impressao }*/)
TRCell():new(oSection2, "NOMECON"  ,"TMP","Descrição da Classe" ,,TAMSX3("CTH_DESC01")[1]+1,/*lPixel*/,/*{|| code-block de impressao }*/)
...
cQuery := "SELECT ZW1_CCUSTO,CTT_DESC01,ZW2_ITEMCO,CTD_DESC01,ZW2_CONTA,CTH_DESC01,ZW2_ANO, "
...
cQuery += "INNER JOIN " +RetSqlName("ZW2")+" ZW2 ON ZW2_CCUSTO = ZW1_CCUSTO AND ZW2_ITEMCO = ZW1_ITEMCO AND ZW2.D_E_L_E_T_ != '*' "
...
cQuery += "WHERE ZW1_FILIAL = '"+XFILIAL("ZW1")+"' "
cQuery += "AND ZW1_CCUSTO BETWEEN '" + MV_PAR01 + "' AND '" + MV_PAR02 + "' "
cQuery += "AND ZW2_ITEMCO BETWEEN '" + MV_PAR03 + "' AND '" + MV_PAR04 + "' "
cQuery += "AND ZW2_CONTA BETWEEN '" + MV_PAR05 + "' AND '" + MV_PAR06 + "' "
cQuery += "AND ZW2_ANO BETWEEN '" + MV_PAR07 + "' AND '" + MV_PAR08 + "' "
cQuery += "AND ZW1.D_E_L_E_T_ != '*' "
cQuery += "ORDER BY ZW2_ANO,ZW1_CCUSTO,ZW2_ITEMCO,ZW2_CONTA "
...
While TMP->(!Eof())
	...
	IncProc("Imprimindo Centro de Custo "+alltrim(TMP->ZW1_CCUSTO))
	oSection1:Cell("CCUSTO"):SetValue(TMP->ZW1_CCUSTO)
   ...
   //	oSection2:Printline()
	...
	While TMP->ZW2_ITEMCO = cItemco
		...
		IncProc("Imprimindo Classe de valor "+alltrim(TMP->ZW2_CONTA))
		
		oSection2:Cell("ITEMCON"):SetValue(TMP->ZW2_ITEMCO)
		oSection2:Cell("CONTA"):SetValue(TMP->ZW2_CONTA)
		oSection2:Cell("NOMECON"):SetValue(TMP->CTH_DESC01)

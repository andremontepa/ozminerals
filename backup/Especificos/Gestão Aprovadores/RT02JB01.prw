...
//====================================================================================================
    ...
    If IsBlind()
		FwLogMsg("RT02JB01", /*cTransactionId*/, "RT02JB01", FunName(), "","[RT02JB01] ["+cEmp+"-"+cFil+"] [" + Dtoc(Date()) +" "+ Time()+ "]  Inicio do processo de Verificacao ...", 0, (nStart - Seconds()), {})
    Else 
		FwLogMsg("RT02JB01", /*cTransactionId*/, "RT02JB01", FunName(), "","[RT02JB01] ["+cEmpAnt+"-"+cFilAnt+"] [" + Dtoc(Date()) +" "+ Time()+ "]  Inicio do processo de Verificacao ...", 0, (nStart - Seconds()), {})
    EndIf 
    ...
    If IsBlind()
		FwLogMsg("RT02JB01", /*cTransactionId*/, "RT02JB01", FunName(), "","[RT02JB01] ["+cEmp+"-"+cFil+"] [" + Dtoc(Date()) +" "+ Time()+ "]  Fim do processo de Verificacao ...", 0, (nStart - Seconds()), {})
    Else 
		FwLogMsg("RT02JB01", /*cTransactionId*/, "RT02JB01", FunName(), "","[RT02JB01] ["+cEmpAnt+"-"+cFilAnt+"] [" + Dtoc(Date()) +" "+ Time()+ "]  Fim do processo de Verificacao ...", 0, (nStart - Seconds()), {})
    EndIf
Return
...
//====================================================================================================
    ...
    If nQtdReg <= 0
		...
		FwLogMsg("RT02JB01", /*cTransactionId*/, "RT02JB01", FunName(), "","[RT02JB01] [BuscaZZY]  [" + Dtoc(Date()) +" "+ Time()+ "] Nao foi encontrada nenhuma regra de substituicao para ser executada ...", 0, (nStart - Seconds()), {})
    Else
		While ! (cAliasZZY)->(Eof())
            ProcGrupo(cAliasZZY)
            (cAliasZZY)->(DbSkip())
...
//====================================================================================================
    ...
    ElseIf (cAliasZZY)->ZZY_STATUS $ "AP/MP" 
        If IsBlind()
            If .not. (Val(Dtos(Date())) > Val((cAliasZZY)->ZZY_DTFIM) .and. (cAliasZZY)->ZZY_TPEXEC == "A")
        		FwLogMsg("RT02JB01", /*cTransactionId*/, "RT02JB01", FunName(), "","[RT02JB01] [ProcGrupo]  [" + Dtoc(Date()) +" "+ Time()+ "] Nao ha regras programadas para executar na data de hoje: "+Dtoc(Date())+" ...", 0, (nStart - Seconds()), {})
                Return Nil 
            EndIf 
        EndIf 
    ...
    If .not. Empty(cQuery)
        ...
        If nQtdReg <= 0
    		...
    		FwLogMsg("RT02JB01", /*cTransactionId*/, "RT02JB01", FunName(), "","[RT02JB01] [ProcGrupo]  [" + Dtoc(Date()) +" "+ Time()+ "] Dados dos Grupos nao Encontrados ...", 0, (nStart - Seconds()), {})
        Else
            If (cAliasZZY)->ZZY_STATUS $ "AA/MA"
                While .not. (cAliasGer)->(Eof())
                    If GetCodAprov(AllTrim((cAliasZZY)->ZZY_SUBST),AllTrim((cAliasGer)->AL_COD),AllTrim((cAliasGer)->AL_NIVEL),AllTrim((cAliasZZY)->ZZY_SUBTMP),(cAliasZZY)->ZZY_STATUS,AllTrim((cAliasZZY)->ZZY_TPEXEC))
                		FwLogMsg("RT02JB01", /*cTransactionId*/, "RT02JB01", FunName(), "","[RT02JB01] [ProcGrupo]  [" + Dtoc(Date()) +" "+ Time()+ "] O Substituto ja se Encontra cadastrado no Grupo de Aprovação da Regra Selecionada ...  [GRUPO: "+AllTrim((cAliasGer)->AL_COD)+"] [USUARIO: "+AllTrim((cAliasGer)->AL_USER) +"] [NIVEL: "+AllTrim((cAliasGer)->AL_NIVEL)+"] [APROVADOR: "+(cAliasGer)->AL_APROV+"] [STATUS: "+(cAliasZZY)->ZZY_STATUS+"]", 0, (nStart - Seconds()), {})
                        lExiste := .T.
                    Else 
                        lExiste := .F. 
                    EndIf 
...
//====================================================================================================
    ...
    For nX := 1 To Len(aDadosCR)
		...
		If (TcSqlExec(cUpd) < 0)
    		FwLogMsg("RT02JB01", /*cTransactionId*/, "RT02JB01", FunName(), "","[RT02JB01][ExecCRTela]  [" + Dtoc(DATE()) +" "+ Time()+ "]  Falha da Execução do UPDATE - SCR ...", 0, (nStart - Seconds()), {})
		Else
    		FwLogMsg("RT02JB01", /*cTransactionId*/, "RT02JB01", FunName(), "","[RT02JB01][ExecCRTela]  [" + Dtoc(DATE()) +" "+ Time()+ "]  Update na Tabela SCR executado com sucesso ...", 0, (nStart - Seconds()), {})
		EndIf
    ...
    If .not. Empty(cUserFim) .and. .not. Empty(cAprovFim)
        ...
        If (TcSqlExec(cUpd) < 0)
    		FwLogMsg("RT02JB01", /*cTransactionId*/, "RT02JB01", FunName(), "","[RT02JB01][ExecCRTela]  [" + Dtoc(DATE()) +" "+ Time()+ "]  Falha da Execução do UPDATE - SAL ...", 0, (nStart - Seconds()), {})
		Else
    		FwLogMsg("RT02JB01", /*cTransactionId*/, "RT02JB01", FunName(), "","[RT02JB01][ExecCRTela]  [" + Dtoc(DATE()) +" "+ Time()+ "]  Update na Tabela SAL executado com sucesso ...", 0, (nStart - Seconds()), {})
		EndIf
    EndIf 
Return 
...
//====================================================================================================
    ...
    If nQtdReg <= 0
		...
		FwLogMsg("RT02JB01", /*cTransactionId*/, "RT02JB01", FunName(), "","[RT02JB01] [ExecAbertos]  [" + Dtoc(Date()) +" "+ Time()+ "] O Aprovador cadastrado na regra nao foi encontrado no grupo de aprovacao ...", 0, (nStart - Seconds()), {})
    Else
        DbSelectArea("SAL")
        SAL->(DbSetOrder(1))
		While ! (cAliasSAL)->(Eof())
...
//====================================================================================================
    ...
    If (cAliasEXC)->(Eof())
		...
		FwLogMsg("RT02JB01", /*cTransactionId*/, "RT02JB01", FunName(), "","[RT02JB01] [ProcSCR]  [" + Dtoc(Date()) +" "+ Time()+ "] Nada na SRC para Processar pelo Filtro -> [GRUPO: "+AllTrim(cGrupo)+"] [USUARIO: "+AllTrim(cUser)+"] [NIVEL: "+AllTrim(cNivel)+"] [APROVADOR: "+AllTrim(cAprov)+"] [STATUS: "+AllTrim(cStatus)+"] ...", 0, (nStart - Seconds()), {})
    Else 
        (cAliasEXC)->(DbGoTop())
        DbSelectArea("SCR")
        SCR->(DbSetOrder(1))
...
//====================================================================================================
    ...
    If (cAliasAll)->(Eof())
		...
		FwLogMsg("RT02JB01", /*cTransactionId*/, "RT02JB01", FunName(), "","[RT02JB01] [PutSCRAll]  [" + Dtoc(Date()) +" "+ Time()+ "] Nada na SRC para Processar pelo Filtro -> [GRUPO: "+AllTrim(cGrupo)+"] [USUARIO: "+AllTrim(cUser)+"] [NIVEL: "+AllTrim(cNivel)+"] [APROVADOR: "+AllTrim(cAprov)+"] [STATUS: "+AllTrim(cStatus)+"] ...", 0, (nStart - Seconds()), {})
    Else 
        (cAliasAll)->(DbGoTop())
        DbSelectArea("SCR")
        SCR->(DbSetOrder(1))

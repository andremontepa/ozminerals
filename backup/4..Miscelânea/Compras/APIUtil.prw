...
//=============================================================================================================================
    ...
    Static Method EnviarEmail(cPara,cAssunto,cMsgEmail,cFilePath)
...
Metodo responsável por enviar um email para notificação do usuario.
@type Method
@author Ricardo Tavares Ferreira
@since 19/09/2022
@param cPara, character, Email para quem vai ser enviado.
@param cAssunto, character, Assunto do Email.
@param cMsgEmail, character, Mensagem do Corpo do Email.
@param cFilePath, character, Caminho do anexo a ser colocado no email.
@history 19/09/2022, Ricardo Tavares Ferreira , Construção Inicial.
@return logical, Retorna verdadeiro se o email foi enviado com sucesso.
/*/
//=====================================================================================================================================
	Method EnviarEmail(cPara,cAssunto,cMsgEmail,cFilePath) class APIUtil // APIUtil():EnviarEmail(cPara,cAssunto,cMsgEmail,cFilePath)
//=====================================================================================================================================

	Local oServer 	    := Nil
    Local oMail         := Nil
	Local nRet 		    := 0
	Local cServer 	    := SubStr(GetMV("MV_RELSERV"),1,AT(":",GetMV("MV_RELSERV")) - 1)
	Local cPorta 	    := SubStr(GetMV("MV_RELSERV"),AT(":",GetMV("MV_RELSERV")) + 1)
	...
	nRet := oServer:Init("",cServer,GetMV("MV_RELAUSR"),GetMV("MV_RELAPSW"),,Val(cPorta))
	If nRet # 0 
		APIUtil():ConsoleLog("APIUtil|EnviarEmail","Não foi possivel Iniciar a Conexão com o Servidor de Email. Erro...: " + CRLF + oServer:GetErrorString(nRet),3)
		Return .F.
	EndIf

	nRet := oServer:SetSMTPTimeout(GetMV("MV_RELTIME"))
	If nRet # 0
		APIUtil():ConsoleLog("APIUtil|EnviarEmail","Timeout de Conexão Exedido. Erro: " + CRLF + oServer:GetErrorString(nRet),3)
		Return .F.
	EndIf

	nRet := oServer:SmtpConnect()
	If nRet # 0
		APIUtil():ConsoleLog("APIUtil|EnviarEmail","Falha ao Conectar-se no Servidor de Email. Erro: " + CRLF + oServer:GetErrorString(nRet),3)
	    oServer:SMTPDisconnect()
		Return .F.
	Endif

	nRet := oServer:SmtpAuth(GetMV("MV_RELAUSR"),GetMV("MV_RELAPSW"))
	If nRet # 0
		APIUtil():ConsoleLog("APIUtil|EnviarEmail","Falha ao realizar a Autenticação no Servidor de Email. Erro: " + CRLF + oServer:GetErrorString(nRet),3)
	    oServer:SMTPDisconnect()
		Return .F.
	Endif
	...
	oMail:clear()
	oMail:cDate    := cValToChar(date())
	oMail:cFrom    := cMailFrom
	oMail:cTo      := cPara
	oMail:cSubject := cAssunto
    oMail:cBody    := cMsgEmail

    if !Empty(cFilePath)
        nRet := oMail:AttachFile(cFilePath)
        If nRet < 0
			APIUtil():ConsoleLog("APIUtil|EnviarEmail","Falha ao Anexar o Arquivo no Email.",3)
            oServer:SMTPDisconnect()
            Return .F.
        Endif
    EndIf
	...
	If oMail # Nil
		nRet := oMail:Send(oServer)
		If nRet # 0
       		APIUtil():ConsoleLog("APIUtil|EnviarEmail","Não foi possível enviar o email para o cliente. Erro: " + CRLF + oMail:getErrorString(nRet),3)
	    Else 
		    APIUtil():ConsoleLog("APIUtil|EnviarEmail","Email enviado para "+Alltrim(cPara)+ ", Data: "+Dtoc(Date())+" "+Time(),1)
            nRet := oServer:SMTPDisconnect()
            If nRet # 0
				APIUtil():ConsoleLog("APIUtil|EnviarEmail","Nao foi possivel conectar ao servidor SMTP: " + oServer:GetErrorString(nRet),3)
            Endif
		Endif
	Endif
Return .T.

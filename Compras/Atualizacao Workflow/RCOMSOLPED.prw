...
For nX := 1 to Len(aFields)
	aDadosCR := FWSX3Util():GetFieldStruct(aFields[nX])
	If Len(aDadosCR) > 0
		Aadd(aHeaderEx,{GetSX3Cache(aDadosCR[1],"X3_TITULO"),aDadosCR[1], X3Picture(aDadosCR[1]),aDadosCR[3],aDadosCR[4],GetSX3Cache(aDadosCR[1],"X3_VALID"),GetSX3Cache(aDadosCR[1],"X3_USADO"),aDadosCR[2],"SCR", GetSX3Cache(aDadosCR[1],"X3_CONTEXT")})
	EndIf 
Next nX

...
IF EMPTY((cServer:=AllTrim(GetNewPar("MV_RELSERV",""))))
	...
	ELSE
		FwLogMsg("EnviEmail", /*cTransactionId*/, "EnviEmail", FunName(), "","Nome do Servidor de Envio de E-mail nao definido no 'MV_RELSERV'", 0, (nStart - Seconds()), {})
	ENDIF
	RETURN .F.
ENDIF
...
IF EMPTY((cAccount:=AllTrim(GetNewPar("MV_RELACNT",""))))
	...
	ELSE
		FwLogMsg("EnviEmail", /*cTransactionId*/, "EnviEmail", FunName(), "","Conta para acesso ao Servidor de E-mail nao definida no 'MV_RELACNT'", 0, (nStart - Seconds()), {})
	ENDIF
	RETURN .F.
ENDIF
...
IF lShedule .AND. EMPTY(cTo)
	IF !lShedule
		FwLogMsg("EnviEmail", /*cTransactionId*/, "EnviEmail", FunName(), "","E-mail para envio, nao informado.", 0, (nStart - Seconds()), {})
	ENDIF
	RETURN .F.
ENDIF
...
IF EMPTY(cFrom)
	...
	ELSE
		FwLogMsg("EnviEmail", /*cTransactionId*/, "EnviEmail", FunName(), "","E-mail do remetente nao definido no cad. do usuario: "+cUser, 0, (nStart - Seconds()), {})
	ENDIF
	RETURN .F.
ENDIF
...
If !lOK
	...
	ELSE
		FwLogMsg("EnviEmail", /*cTransactionId*/, "EnviEmail", FunName(), "","Falha na Conexใo com Servidor de E-Mail", 0, (nStart - Seconds()), {})
	ENDIF
ELSE
	If lAutentica
	...
	If !lOK
		...
		ELSE
			FwLogMsg("EnviEmail", /*cTransactionId*/, "EnviEmail", FunName(), "","Falha no Envio do E-Mail: "+ALLTRIM(cTo), 0, (nStart - Seconds()), {})
		ENDIF
	...
	Else
		...
		ELSE
			FwLogMsg("EnviEmail", /*cTransactionId*/, "EnviEmail", FunName(), "","E-mail enviado com sucesso: "+ALLTRIM(cTo), 0, (nStart - Seconds()), {})
		ENDIF		
	ENDIF
ENDIF

...
#DEFINE ROTINA_FILE				"OZ06M02.prw"
...
#Define ALIAS_FORM0 			"PAJ" 
#Define MODELO					"OZ06M02"
#Define ID_MODEL				"OZ06A02"
#Define TITULO_MODEL			"Cadastro Contabil Protheus Para Uso Argo - OZMinerals" + SubStr(VERSAO_ROTINA,1,17)
...
/*/{Protheus.doc} OZ06M02 - Cadastro Contabil Protheus Para Uso Argo - OZMinerals

	Função responsável Cadastro Contabil Protheus Para Uso Argo - OZMinerals
...
/*/ 
User Function OZ06M02(aAutoCab,nOperacao)
...
Static Function ModelDef()
	...
	Local cCpoCentroCusto	:= cPrefForm+"_CC"
	Local cCpoItemCtb		:= cPrefForm+"_ITEM"
	Local cCpoClasseValor	:= cPrefForm+"_CLVL"
	Local cCpoDebConta   	:= cPrefForm+"_CTAD"
	Local cCpoCredConta   	:= cPrefForm+"_CTAC"
	...
	oModel:SetPrimaryKey( { cCpoFilial, cCpoCentroCusto, cCpoItemCtb, cCpoClasseValor, cCpoDebConta , cCpoCredConta  } )
...
Static Function posValid(oModel)
	...
	If nOperation = MODEL_OPERATION_INSERT
		...
		Begin Sequence
			...
			If ( (cAliasForm)->(dbSeek( PAD(cCpoFilial      ,TAMSX3("PAJ_FILIAL") [1]) +; 
							  			PAD(cCpoCentroCusto ,TAMSX3("PAJ_CC")     [1]) +;
							  			PAD(cCpoItemCtb     ,TAMSX3("PAJ_ITEM")   [1]) +;
							  			PAD(cCpoClasseValor ,TAMSX3("PAJ_CLVL")   [1]) +;
						  	  			PAD(cCpoDebConta    ,TAMSX3("PAJ_CTAD")   [1]) +;
							  			PAD(cCpoCredConta   ,TAMSX3("PAJ_CTAC")   [1]) )))
...
Static Function showLogInConsole(cMensagem,lAtivaLog,cPrefixo)
	...
	Default cMensagem 	:= ""
	Default cPrefixo	:= "[OZ06M02][" + DtoC(DDATABASE) + "][" + Time() + "] "	
Return

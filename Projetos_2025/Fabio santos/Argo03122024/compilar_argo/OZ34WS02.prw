...
@Obs 
	...
	Somente será gerado o adiantamento Se o status for awaitingIssue

@See u_JobPgtoPAArgo()	

@nested-tags:Frameworks/OZminerals
/*/ 
User Function OZ34WS02()
	...
	If ( oRest:Post(aHeader) )
		...
		u_TGETLIST(cToken)             // CHAMA O METODO DE LISTA
		If ( lExibeToken )
...
User Function TGETLIST(cToken)
	...
	cDt001          		:= CtoD("")
	cDt002          		:= CtoD("")
	cIniData                := CtoD(GetNewPar("OZ_DTIARGO","08/10/2024")) 
		...
		For nx := 1 To Len(aVetor)
			...
			If ( lPermiteExecutar )
				If (dDataPA >= cDt001 .And. dDataPA <= cDt002 )
					...
					EndIf
				Else 
					cRet1  := "O Status "+cStatus+" é diferente de awaitingIssue, somente este status Grava o PA"
...
Static Function XGETDADOS2(cID,cToken,cCliID,cCostCenter)
	...
	If ( lSucess3 := oRest3:Get(aHeader3) )
		...
		If ( !lIssue ) 
			cRet3:= "Tipo Pagamento é o status "+cStatus+" do Argo, diferente de awaitingIssue ou awaitingPayment"
...
Static Function XGETDADOS3(cID, cToken, cCliID, cCostCenter, lIssue )
	...
	If ( lSucess4 := oRest4:Get(aHeader4) )
		...
		If ( lIssue )
			u_TGetDados4(cID,cToken,cCliID,ccpf,cuserid,cEmail,cCostCenter) // CHAMA O METODO QUE RETORNA DADOS PRO REEMBOLSO
		EndIf 

		If ( !lIssue )
			cRet4 := "Este Rengistro Não é Adiantamento Aprovado e Nem Reembolso." + CRLF
			cRet4 += "Será necessario consultar no ARGO de tem os seguintes status:" + CRLF
			cRet4 += "status : awaitingIssue ( Foi aprovado e não há pagemnto ainda)" + CRLF
...
*/
User Function TGetDados4(cID,cToken,cCliID,ccpf,cuserid,cEmail,cCostCenter)
		...
		If ( cAdt )
			If Len(oRest5) > 0
				...
				If ( Alltrim(cEmpTit) == Alltrim(cEmpAnt) )
					If ( lPassa )
						u_TGeraPA(nValor,ccpf,cuserid,cID,dEmiss,cHIst,cEmail,cEmpTit,cFilTit,cCostCenter,cCredConta,cDebConta,cCCusto,cItemCta,cClasseVlr) // CHAMA A FUNÇÃO QUE GERA PA
					Else 
					...
					EndIf 
				Else 
					cLog := "Esta Empresa do é "+cEmpTit+" No Argo, e no Protheus é "+cEmpAnt+", Portanto, não será carregado no Protheus, Favor Verificar se a Empresa Existe!"
					ShowLogInConsole(cLog)
					Return
				EndIf
			EndIf
...
/*/{Protheus.doc} TGeraPA

    FUNÇÃO QUE GERA TITULO DE PAGAMENTO ANTECPADO NO FINANCEIRO
...
@nested-tags:Frameworks/OZminerals
/*/ 
User Function TGeraPA(nValor,ccpf,cuserid,cID,dEmiss,cHIst,cEmail,cEmpTit,cFilTit,cCostCenter,cCredConta,cDebConta,cCCusto,cItemCta,cClasseVlr)
			...
			If ( lReckLock )
				Begin Transaction
					If ( RecLock("ZZ1",lReckLock) )
						...
						ZZ1->ZZ1_STS     := "2"
						ZZ1->ZZ1_HIST    := cHist
						ZZ1->ZZ1_OBS     := "Dados Bancarios não encontrados! Titulo não foi gerado!"
						ZZ1->(MsUnlock())
					EndIf
				End Transaction
			EndIf
			cLog := "Dados Bancarios não encontrados! Titulo não foi gerado!"
			ShowLogInConsole(cLog)
		Endif
	Else
		If ( ZZ1->(dbSeek(  PAD(xFilial("ZZ1")  ,TAMSX3("ZZ1_FILIAL") [1]) +; 

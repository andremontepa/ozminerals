...
#DEFINE ROTINA_FILE				"OZ06M08.prw"
...
#Define ALIAS_FORM0 			"ZZ1" 
#Define MODELO					"OZ06M08"
#Define ID_MODEL				"OZ06A08"
#Define TITULO_MODEL			"Status Integração - Argo X Protheus - OZMinerals" + SubStr(VERSAO_ROTINA,1,17)
...
/*/{Protheus.doc} OZ06M08 - Status Integração - Argo X Protheus - OZMinerals

	Função responsável Status Integração - Argo X Protheus - OZMinerals
...
/*/ 
User Function OZ06M08(aAutoCab,nOperacao)
	...
	Else	
		...
		oFwMBrowse:AddLegend( cAliasForm+"_STS == '1'  ", "BR_VERDE   " , "Integrado" )
		oFwMBrowse:AddLegend( cAliasForm+"_STS == '2'  ", "BR_VERMELHO" , "Não Integrado" )
		oFwMBrowse:AddLegend( cAliasForm+"_STS == '3'  ", "BR_AZUL    " , "Pago" )
...
Static Function ModelDef()
	...
	Local cCpoCodigoID    	:= cPrefForm+"_IDREQ"
	Local cCpoParce      	:= cPrefForm+"_PARCE"
	Local cCpoStatus      	:= cPrefForm+"_STATUS"
	...
	oModel:SetPrimaryKey( { cCpoFilial, cCpoCodigoID, cCpoParce, cCpoStatus } )
...
Static Function posValid(oModel)
	...
	If nOperation = MODEL_OPERATION_INSERT
		...
		Begin Sequence
			...
			If ( (cAliasForm)->(dbSeek( PAD(cCpoFilial      ,TAMSX3("ZZ1_FILIAL") [1]) +; 
							  			PAD(cCpoCodigoID    ,TAMSX3("ZZ1_IDREQ")  [1]) +;
							  			PAD(cCpoParce       ,TAMSX3("ZZ1_PARCE")  [1]) +;
							  			PAD(cCpoStatus      ,TAMSX3("ZZ1_STATUS") [1]) )))
...
Static Function showLogInConsole(cMensagem,lAtivaLog,cPrefixo)
	...
	Default cMensagem 	:= ""
	Default cPrefixo	:= "[OZ06M08][" + DtoC(DDATABASE) + "][" + Time() + "] "	
Return

				...
				If ( Len(aLinha) > 0 .And. nLinhaAtual >= 2 )
					...
					If ( lGravaTabela )
						...
						If ( SB1->(dbSeek(xFilial("SB1") + cCodigoProduto )) )
							...
							If (SDQ->(DbSeek(xFilial("SDQ") + PAD(cCodigoProduto,TAMSX3("DQ_COD") [1])+; 
															  PAD(cArmazem,TAMSX3("DQ_LOCAL")     [1])+; 
															  PAD(DtoS(dDataInventario),TAMSX3("DQ_DATA") [1]))))
								lReckLock := .F.
							Else
								lReckLock := .T.
							EndIf
							...
							If (!lReckLock) 
								Begin Transaction
									...
									MSExecAuto({|x,y,z| MATA338(x,y)},aItem,5)
									If ( !lMsErroAuto )

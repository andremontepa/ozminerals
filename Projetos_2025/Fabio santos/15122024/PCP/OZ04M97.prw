...
User Function OZ04M97()
	...
	cTitoDlg    	         := "Ajuste VALOR 0.01 do inventario - OzMinerals"
	cSeparador               := AllTrim(GetNewPar("OZ_DELIMIT",";"))
	cCtaDespInvent           := AllTrim(GetNewPar("OZ_CTADESP","610118001"))

	aAdd(aSays, "Esta rotina tem por objetivo ajustar VALOR 0.01 do inventario.")
					...
					If ( nVlrUnitario > 0 )
						...
						If ( lGravaTabela )
							...
							If ( SB1->(dbSeek(xFilial("SB1") + cCodigoProduto )) )
								...
								//BuscaIndiceMovimentacao(cCodFilial, cCodigoProduto, cArmazem ,dDataInventario, nQuantidade)
								...
								If ( SB1->(dbSeek(xFilial("SB1") + cCodigoProduto )) )
									...
									aAdd(aItem, {"D3_LOCAL"  ,cArmazem             ,  Nil})
									aAdd(aItem, {"D3_CUSTO1" ,nQuantidade * 0.01   ,  Nil})
									aAdd(aItem, {"D3_CUSTO2" ,nQuantidade * 0.0543 ,  Nil})
									aAdd(aItem, {"D3_CONTA"  ,cCheckCtaCtb         ,  Nil})
									aAdd(aItem, {"D3_CC"     ,cCentroCusto         ,  Nil})
									aAdd(aItem, {"D3_ITEMCTA",cItemContabil        ,  Nil})
									aAdd(aItem, {"D3_CLVL"   ,cClasseValor         ,  Nil})
									aAdd(aItem, {"D3_XCTADES",cCtaDespInvent       ,  Nil})

...
Local cEmailSoli  := ""
    ...
    ConOut('consulta os dados da inclusใo do titulo')
    ...
    cNum := (cSQL)->E2_NUM
    ConOut('Busca informa็๕es do solicitante')
    ...
    */
    (cSQL)->(DbGoTop())
    Do While (cSQL)->(!Eof())
        ConOut('Busca e-mail do aprovador: ' + (cSQL)->ZC_APROVA)
        ...
        If !Empty(cEmailAprv)
            ConOut('E-mail do aprovador selecionado: ' + cEmailAprv)
            cMail := cEmailAprv
...
ConOut('Entrou na rotina ' + cnumero)
If (oProcess:oHtml:RetByName('RBAPROVA') == 'Si')
	ConOut('Autorizo o conta a pagar N: '+ cnumero)
	ConOut('cMes :'+cMes)
    ...
    ConOut('Flag da libera็ใo' + str(nFlag) + ' Nivel ' + cNnivel)
    ...
    ConOut('Verifica se exitem mais aprovadores')
    iF !Empty(SZCTB->ZC_NIVEL) 
        ConOut('mais aprovadores encontrados')
        oProcess := TWFProcess():New( "000002", "Solicta็ใo contas a pagar" )   
    ...
    Else 
        ...
        cUpdate += "AND D_E_L_E_T_ = ' ' "
        nFlag := TcSqlExec(cUpdate)                
        ConOut('Flag da libera็ใo SE2 ' + str(nFlag) + ' Nivel ' + cNnivel)
...
Local cEmailSoli  := ""
    ...
    ConOut('Proximo aprovador:  consulta os dados da inclusใo do titulo *************')
    cQuery := " SELECT TOP 1 E2_FILIAL,E2_PREFIXO,E2_NUM,E2_PARCELA,E2_TIPO,E2_FORNECE, "
    cQuery += " E2_LOJA,SA2.A2_NOME E2_NOMFOR,E2_NATUREZ,E2_EMISSAO,E2_VENCTO,E2_VALOR,E2_HIST, "
    cQuery += " ZC_GRUPO,ZC_APROVA,ZC_NIVEL "
    cQuery += " FROM " + RetSqlName("SE2") + " SE2 "
    ...
    dbUseArea(.T.,"TOPCONN",TcGenQry(,,cQuery),cSQL,.T.,.T.) 
    cNum := (cSQL)->E2_NUM
    ConOut('Busca informa็๕es do solicitante ' + _cSol)
    PswOrder(2)    // ordenado pelo nome
    If PswSeek(_cSol, .T. )
        aInfo := PswRet()
        cIDSolicit  := alltrim(aInfo[1][1])

        ...
        For nX := 1 To Len(aRegFim)
            RecLock(cTab,.T.)
                For nY := 1 To Len(aRegFim[nX])
                    /*If alltrim(aRegFim[nX][nY][1]) == 'N1_CBASE' .or. alltrim(aRegFim[nX][nY][1]) == 'N3_CBASE'
                        If len(aRegFim[nX][nY][2]) > 10
                           aRegFim[nX][nY][2]:=  Substr(aRegFim[nX][nY][2],len(aRegFim[nX][nY][2])-9)
                        EndIf
                    EndIf*/

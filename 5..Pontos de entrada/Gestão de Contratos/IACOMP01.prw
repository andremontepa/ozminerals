	...
	If CN9->CN9_XGLOBA == "N"
		...
		If SCR->(dbSeek(CND->CND_FILIAL + "PC" + CND->CND_PEDIDO))
			Do While !SCR->(Eof()) .And. SCR->CR_FILIAL + SCR->CR_TIPO + Alltrim(SCR->CR_NUM) == CND->CND_FILIAL + "PC" + CND->CND_PEDIDO
				//Alert("Deletando pedido scr " + CND->CND_PEDIDO)
...
//	Local nXj := 0
	...
	Local _cEmpDes := CND->CND_XEMPRE
	Local _cFilDes := CND->CND_XFILIA
	Local aLisCmp := {"CH_FILIAL","CH_PEDIDO","CH_FORNECE","CH_LOJA","CH_ITEMPD","CH_ITEM","CH_PERC","CH_CC","CH_ITEMCTA","CH_CLVL","CH_CUSTO1"}
				...
				Do While !CNZ->(Eof()) .And. CNE->CNE_FILIAL + CNE->CNE_CONTRA + CNE->CNE_REVISA + CNE->CNE_NUMMED + CNE->CNE_ITEM == CNZ->CNZ_FILIAL + CNZ->CNZ_CONTRA + CNZ->CNZ_REVISA + CNZ->CNZ_NUMMED + CNZ->CNZ_ITCONT
					...
					For nXi := 01 to Len(aLisCmp)
						...
						ElseIf Alltrim(aLisCmp[nXi]) == "CH_ITEMPD"
							cItePed := Posicione("SC7",4,xFilial("SC7") + CNE->CNE_PRODUT + CNE->CNE_PEDIDO,"C7_ITEM")
							//Alert("CH_ITEMPD " + cItePed)
							cQryIns +=  "'" + cItePed + "',"
							//cQryIns +=  "'" + StrZero(Val(CNZ->CNZ_ITCONT),04) + "',"
						...
						ElseIf Alltrim(aLisCmp[nXi]) == "CH_ITEMCTA"
							cQryIns +=  "'" + CNZ->CNZ_XITEM + "',"
						ElseIf Alltrim(aLisCmp[nXi]) == "CH_CLVL"
							cQryIns +=  "'" + CNZ->CNZ_XCLVL + "',"
...
IIF(SD1->D1_RATEIO<>"1",IIF(SD1->D1_VALCOF>0 .AND. SD1->D1_VALPIS>0 .AND. SD1->D1_VALCSL>0, SD1->D1_VALCOF+SD1->D1_VALPIS+SD1->D1_VALCSL,0), 0) 

IIF(SD1->D1_RATEIO<>"1" .AND. !SD1->D1_TES $ "004-206", SD1->D1_CUSTO - IIf(SF4->F4_ATUATF='N', SD1->D1_ICMSCOM,0), 0)                                                                                  

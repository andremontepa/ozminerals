...
Static Function MenuDef()
	...
	ADD OPTION aRotina TITLE "Gera Saldo Inicial"         ACTION "u_OZ04M33()"        OPERATION 10  ACCESS 0
	ADD OPTION aRotina TITLE "Realiza  Backup Lote 8840"  ACTION "u_OZ04M28()"        OPERATION 10  ACCESS 0
	ADD OPTION aRotina TITLE "Restaura Backup Lote 8840"  ACTION "u_OZ04M29()"        OPERATION 10  ACCESS 0
...
Static Function LinePreValid(oModelGrid, nLinha, cAcao, cCampo)
	...
	Local cCpoAvanco 	:= cPrefGrid+"_AVANCO"
	Local nOperation	:= oModel:GetOperation()
	Local cStatus		:= ""  as character 
	Local cAvanco 		:= ""  as character 
	...
	If ( nOperation == MODEL_OPERATION_UPDATE )	
		If ( cAcao == "SETVALUE")  
			If ( !oModelGrid:IsDeleted() )
				...
				cAvanco     := oModelGrid:GetValue(cCpoAvanco)
				If (Alltrim(cStatus) $ Alltrim(GERADO_SUCESSO) .And. Alltrim(cAvanco) $ Alltrim(MOV_APONTAROP) )
					lRet :=  .F.
					Help(,,"LinepreValid",,"Atenção! Registro gerado com sucesso, não é permitido alteração!",1,0)
				ElseIf (Alltrim(cStatus) $ Alltrim(GERADO_SUCESSO) .And. Alltrim(cAvanco) $ Alltrim(MOV_TRANSFERIR) )
					lRet :=  .F.
					Help(,,"LinepreValid",,"Atenção! Registro gerado com sucesso, não é permitido alteração!",1,0)
				ElseIf (Alltrim(cStatus) $ Alltrim(GERADO_SUCESSO) .And. Alltrim(cAvanco) $ Alltrim(MOV_BAIXAREQ) )
					lRet :=  .F.
					Help(,,"LinepreValid",,"Atenção! Registro gerado com sucesso, não é permitido alteração!",1,0)
				ElseIf (Alltrim(cStatus) $ Alltrim(GERADO_SUCESSO) .And. Alltrim(cAvanco) $ Alltrim(MOV_DOCGERADO) )
					lRet :=  .F.
					Help(,,"LinepreValid",,"Atenção! Registro gerado com sucesso, não é permitido alteração!",1,0)
				EndIf
			EndIf
...
/*/{Protheus.doc} OZ04M33 - Gestão Automatica de Producação 

	Função responsável ajustar os saldos iniciais do custo em partes
...
@nested-tags:Frameworks/OZminerals
/*/ 
User Function OZ04M33() 
	Local aArea             := {}  as array
	Local lPermiteExecutar  := .F. as Logical

	aArea                   := GetArea()
	lPermiteExecutar        := GetNewPar("OZ_MA280FI",.T.)

	If ( !IsBlind() )
		If ( lPermiteExecutar )
			If ( FindFunction("estoque.Producao.Custeio.u_SaldoFinalCustoEmPartes") )
				estoque.Producao.Custeio.u_SaldoFinalCustoEmPartes(cDataFechamento)
			EndIf
		EndIf
	EndIf

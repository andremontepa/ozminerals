...
User function IACOMP01()
	...
	If CN9->CN9_XGLOBA == "N"  // Somente Contratos Nacionais
		...
		CND->(MSUNLOCK())
		  ...
		  //%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%//
  		  // POSICIONA PRIMEIRO ITEM CNE E ATUALIZA ENTIDADES CONTÁBEIS  //
		  //            Julio Martins - CRMServices - Jul-2025           //
   		  //%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%//
                        ...
                        Dbsetorder(8) 
							...
							RecLock("CNZ", .F.)
							if Empty(CNZ->CNZ_XCC)
							CNZ->CNZ_XCC:=cCustoz
							Endif
							If Empty(CNZ->CNZ_XITEM)
							CNZ->CNZ_XITEM:=cItemoz
							Endif
							If empty(CNZ->CNZ_XCLVL)
							CNZ->CNZ_XCLVL:=cClvloz
							Endif
							CNZ->(Msunlock())
							Endif
		 					Dbselectarea("CNE")
                      		Dbsetorder(5)  // Filial - Contrato - Revisão - Numero - Nº Medição - Item Medição
                        	if Dbseek(xFilial()+cContra+cRevisa+cMedica)  
						   	If empty(CNE->CNE_CC)                                                                                                                                    
                        	RecLock("CNE", .F.)
                        	CNE->CNE_CC:=cCustoz
                       		CNE->CNE_ITEMCT:=cItemoz
                        	CNE->CNE_CLVL:=cClvloz
                        	CNE->CNE_XCC:=cCustoz
                        	CNE->CNE_XITCTA:=cItemoz
                        	CNE->CNE_XCLVL:=cClvloz
                        	CNE->(Msunlock())
							Endif
                        	Endif 
...
While  SC7->C7_PRODUTO=CNE->CNE_PRODUT .and. SC7->C7_NUM=CNE->CNE_PEDIDO

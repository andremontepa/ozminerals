...
/*/{Protheus.doc} RT02R001

Progrma que realiza a Impressao do Pedido de Compra em HTML.
...
//==========================================================================================================  
	...
	If FunName() == "MATA121"
		...
		If GET_DAD()
			...
			TMP2->(DbCloseArea())
		Else
			Aviso("XT02R002","Não Há dados para o Pedido de Compra Informado.",{"Fechar"},1)
		EndIf
	...
	Else
		If GeraPar()
			If GET_DAD()
				...
				TMP2->(DbCloseArea())
			Else
				Aviso("RT02R001","Não Há dados para o Pedido de Compra Informado.",{"Fechar"},1)
			EndIf
		EndIf
	EndIF
...
/*/{Protheus.doc} PROC_DAD
Funcao que realiza a criaçao do html e a impressao do mesmo..
@author 	Ricardo Tavares Ferreira
@since 		02/03/2018
@version 	12.1.17
@return 	Logico
...
//==========================================================================================================
	...
	//FWrite(nHandle,'  						<h6>Caixa Postal Parauapebas N. 68 - CEP: 68.515-000</h6> '+QbLinha)
	//FWrite(nHandle,'  						<h6>Caixa Postal Curionopolis N. 122 - CEP: 68.523-000</h6> '+QbLinha)
	FWrite(nHandle,'  					</td> '+QbLinha)
	...
	//If GET_RAT(cCodRat,cFilC7,cNumPed) > 0
		...
		FWrite(nHandle,'  				</tr> '+QbLinha)
		FWrite(nHandle,'  				<tr> '+QbLinha)
		FWrite(nHandle,'  					<td align = "center"><b>Item Pedido</b></td> '+QbLinha)
		...
		//If cControla =="1"
		FWrite(nHandle,'  					<td align = "center"><b>Projeto</b></td> '+QbLinha)
		FWrite(nHandle,'  					<td align = "center"><b>Tarefa</b></td> '+QbLinha)
		//endif
		FWrite(nHandle,'  				</tr> '+QbLinha)
		...
		While ! TMP2->(EOF())
			...
			FWrite(nHandle,'  				<tr> '+QbLinha)
			FWrite(nHandle,'  					<td align = "center">'+Alltrim(TMP2->ITEM)+'</td> '+QbLinha)
			...
			//If cControla =="1"
			FWrite(nHandle,'  					<td align = "center">'+Alltrim(TMP2->PROJET)+'</td> '+QbLinha)
			FWrite(nHandle,'  					<td align = "center">'+Alltrim(TMP2->TAREFA)+'</td> '+QbLinha)
			//endif
		...
		GET_RAT1(cCodRat,cFilC7,cNumPed) 
	...
	FWrite(nHandle,'  				</tr> '+QbLinha)
	FWrite(nHandle,'  				<tr> '+QbLinha)
	FWrite(nHandle,'  					<td><h6>Enviar NF-e para o E-mail: nfe.carajas@ozminerals.com;</h6></td> '+QbLinha)
...
/*/{Protheus.doc} GET_DAD
Funcao que realiza a busca dos dados no Banco de Dados para a impressao do relatorio.
@author 	Ricardo Tavares Ferreira
@since 		02/03/2018
@version 	12.1.17
@return 	Logico
...
//==========================================================================================================
	...
	//cQuery += " , ISNULL(CONVERT(VARCHAR(1024),CONVERT(VARBINARY(1024),C7_XOBSF)),'') OBS "+QbLinha
	...
	Endif
		cQuery += " ORDER BY C7_ITEM "+QbLinha
...
//==========================================================================================================
	...
	Public cControla    := ""
		...
		cQuery += ", CH_PROJET PROJET "+QbLinha
		cQuery += ", CH_TAREFA TAREFA "+QbLinha
		cQuery += ", CH_ITEMPD ITEM "+QbLinha
		...
		//cQuery += "AND SC7.C7_RATEIO = '1' "+QbLinha
		cQuery += "WHERE "+QbLinha
		cQuery += "SCH.D_E_L_E_T_ = ' ' "+QbLinha
		cQuery += "AND CH_FILIAL = '"+cEmp+"' "+QbLinha
		cQuery += "AND CH_PEDIDO = '"+cNum+"' "+QbLinha

		cQuery += "GROUP BY CH_ITEMPD, CH_CC, CH_ITEMCTA, CH_PROJET, CH_TAREFA, CH_PERC, C7_IPI
		cQuery += "ORDER BY CH_ITEMPD "+QbLinha



	MEMOWRITE("C:/ricardo/XT02R00XA.sql",cQuery)
	cQuery := ChangeQuery(cQuery)
...
/*/
//==========================================================================================================
Static Function GET_RAT1(cRat,cEmp,cNum)
//==========================================================================================================
	...
	Public cControla    := ""
		...
		cQuery += ", C7_PROJET PROJET "+QbLinha
		cQuery += ", C7_TAREFA TAREFA "+QbLinha
		cQuery += ", C7_ITEM ITEM "+QbLinha
		...
		cQuery += "AND SC7.C7_RATEIO = '2' "+QbLinha
		//cQuery += "AND C7_ITEM = '"+cItem+"' "+QbLinha
		cQuery += "GROUP BY C7_ITEM, C7_CC, C7_ITEMCTA, C7_PROJET, C7_TAREFA, C7_IPI "+QbLinha
		cQuery += "ORDER BY C7_ITEM "+QbLinha
...
/*/{Protheus.doc} GET_PAR
Funcao que realiza a Geração dos Parametros Para a impressao do Relatorio.
@author 	Ricardo Tavares Ferreira
@since 		18/03/2018
@version 	12.1.17
@return 	Logico
...
//==========================================================================================================
	...
	AADD(aParamBox,{1,"Filial"		,Space(TamSX3("C7_FILIAL")[1])		,"","","SM0","",0,.T.}) //MV_PAR01
	AADD(aParamBox,{1,"Pedido"		,Space(TamSX3("C7_NUM")[1])			,"","","SC7","",0,.T.}) //MV_PAR02

	// Verificacao se o Array gravavel.

...
/*/{Protheus.doc} OZ34M09

	Rotina atualizar Cadastro Centro Custo do Argo 
...
User Function OZ34M09()
	...
	cTitoDlg    	              := "OZMinerals - Cadastro Centro Custo do Argo"

	aAdd(aSays, "Esta rotina tem por objetivo dar carga no Cadastro do Argo Centro Custo!")
...
Static Function ImportaArquivo(cArqSelecionado)
	...
	Local cCodEmpID          := ""  as character
	Local cCodCentroArgo     := ""  as character 
	Local cDescCustoArgo     := ""  as character 
	...
	Local c09Argo            := ""  as character
	Local c10Argo            := ""  as character
	...
	oExcel:AddColumn(cNomePlanilha, cTituloPlanilha, "Desc. Id Argo"          , 1, 1, .F.)
	oExcel:AddColumn(cNomePlanilha, cTituloPlanilha, "Codigo C.Custo Argo"    , 1, 1, .F.)
	oExcel:AddColumn(cNomePlanilha, cTituloPlanilha, "Desc. Centro Custo Argo", 1, 1, .F.)
	...
	oExcel:AddColumn(cNomePlanilha, cTituloPlanilha, "Centro Custo"           , 1, 1, .F.)
	oExcel:AddColumn(cNomePlanilha, cTituloPlanilha, "Desc. Centro Custo"     ,1, 1, .F.)
		...
		If ( !(oArquivo:EoF()) )
			...
			While (oArquivo:HasLine())
				...
				If ( Len(aLinha) > 0 .And. nLinhaAtual >= 2 )
					...
					cCodID             := aLinha[01]
					cEmpresa           := aLinha[02]
					cCodigoFilial      := aLinha[03]
					cCodEmpID          := aLinha[04]
					cCodCentroArgo     := aLinha[05]
					cItemContabil      := aLinha[06]
					cCentroCusto       := aLinha[07]
					cDescCustoArgo     := aLinha[08]
					...
					If ( lGravaTabela )
						...
						c02Argo   := AllTrim(cEmpresa) 
						c03Argo   := AllTrim(cCodigoFilial) 
						c04Argo   := AllTrim(cCodEmpID)
						c05Argo   := AllTrim(cCodCentroArgo)
						c06Argo   := AllTrim(cItemContabil)
						c07Argo   := AllTrim(Posicione("CTD",1,xFilial("CTD")+cItemContabil ,"CTD_DESC01"))
						c08Argo   := AllTrim(cCentroCusto)
						c09Argo   := AllTrim(Posicione("CTT",1,xFilial("CTT")+cCentroCusto ,"CTT_DESC01"))
						c10Argo   := AllTrim(cDescCustoArgo)

						dbSelectArea("PAK")
						PAK->(dbSetOrder(1))
						If ( PAK->(dbSeek(PAD(xFilial("PAK") ,TAMSX3("PAK_FILIAL") [1]) +; 
										  PAD(c01Argo        ,TAMSX3("PAK_ID")     [1]) )))
							lReckLock := .F.
						Else 
							lReckLock := .T.
						EndIf
						...
						If ( lReckLock )
							Begin Transaction
								PAK->(RecLock("PAK",lReckLock))
									PAK->PAK_FILIAL := xFilial("PAJ")
									PAK->PAK_ID     := c01Argo
									PAK->PAK_EMP    := c02Argo
									PAK->PAK_FIL    := c03Argo
									PAK->PAK_EMPID  := c04Argo
									PAK->PAK_CODCC  := c05Argo
									PAK->PAK_ITEM   := c06Argo
									PAK->PAK_DSCITC := c07Argo
									PAK->PAK_CC     := c08Argo
									PAK->PAK_DSCCC  := c09Argo
									PAK->PAK_DSCARG := c10Argo
									PAK->PAK_MSBLQL := "2"
								PAK->(MsUnlock())
							End Transaction
						EndIf
					...
					EndIf
				Else 
					cStatus        := "Empresa Não Localizada" 
					lAbre := .T.
				EndIf

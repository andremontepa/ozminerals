...
#DEFINE ROTINA_FILE				"OZ04M18.prw"
...
#Define ALIAS_FORM0 			"PAW" 
#Define MODELO					"OZ04M18"
#Define ID_MODEL				"OZ04A18"
#Define TITULO_MODEL			"Cadastro Operação Contabil - OZMinerals" + SubStr(VERSAO_ROTINA,1,17)
...
/*/{Protheus.doc} OZ04M18 - Cadastro Operação Contabil

	Função responsável Cadastro Operação Contabil
...
/*/ 
User Function OZ04M18(aAutoCab,nOperacao)
	...
	Else	
		...
		oFwMBrowse:AddLegend( cAliasForm+"_MSBLQL == '1'  ", "BR_AZUL    " , "Bloqueado" )
...
Static Function ModelDef()
	...
	Local cCpoFilCod		:= cPrefForm+"_COD"
	...
	oModel:SetPrimaryKey( { cCpoFilial, cCpoFilCod } )
...
Static Function posValid(oModel)
	...
	Local cFilCod	    := oModel:GetValue(cIDModelForm,cCpoFilCod)	
	...
	If nOperation = MODEL_OPERATION_INSERT
		...
		Begin Sequence
			...
			If (cAliasForm)->(dbSeek(cFilLogada+cFilCod))
...
Static Function showLogInConsole(cMensagem,lAtivaLog,cPrefixo)
	...
	Default cMensagem 	:= ""
	Default cPrefixo	:= "[OZ04M18][" + DtoC(DDATABASE) + "][" + Time() + "] "	
Return

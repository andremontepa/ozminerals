...
/*/{Protheus.doc} OZ34WS01 - Manual

    WEB SERVICE DE REEMBOLSO - ARGO - OZminerals 
	Esta rotina Será executado de forma manual quando necessario pelo usuário  
...
User Function OZ34WS01()
	...
	If ( oRest:Post(aHeader) )
		...
		u_ZGETLIST(cToken)             // CHAMA O METODO DE LISTA
		If ( lExibeToken )
...
User Function ZGETLIST(cToken)
	...
	//cGetParam += "&requestNumber="+AllTrim(cCodTit)
	cGetParam += "&products=refund"
	cGetParam += "&pages=3"
	//LUCAS RAMINELLI - 02/05/2025 - incluido o filtro de alteracao para nao trazer itens antigos
	cGetParam += "&initialChangeDate=" + cDtIni + "T00:00:00.000Z"
	cGetParam += "&finalChangeDate=" + cDtFim + "T23:59:00.000Z"
...
Static Function XGETDADOS3(cID,cToken,cCliID,cCostCenter)
	...
	If ( lSucess4 := oRest4:Get(aHeader4) )
		...
		u_ZGETDADOS4(cID,cToken,cCliID,ccpf,cuserid,cEmail,cCostCenter) // CHAMA O METODO QUE RETORNA DADOS PRO REEMBOLSO
...
@nested-tags:Frameworks/OZminerals
/*/ 
User Function ZGETDADOS4(cID,cToken,cCliID,ccpf,cuserid,cEmail,cCostCenter)
		...
		If ( lPermite )
			If ( Len(oRest5) > 0 )
				...
				If ( Alltrim(cEmpTit) == Alltrim(cEmpAnt) )
					If ( lPassa )
						u_ZGERATIT(nValor,ccpf,cuserid,cID,dEmiss,cHIst,cEmail,cCostCenter,cEmpTit,cFilTit,cCredConta,cDebConta,cCCusto,cItemCta,cClasseVlr) // CHAMA A FUNÇÃO QUE GERA O TITULO
					Else 
...
@nested-tags:Frameworks/OZminerals
/*/ 
User Function ZGERATIT(nValor,ccpf,cuserid,cID,dEmiss,cHIst,cEmail,cCostCenter,cEmpTit,cFilTit,cCredConta,cDebConta,cCCusto,cItemCta,cClasseVlr)
	Local aArea         := {}  as array
	...
	//Lucas Raminelli - Possivel foco erro fornecedor
	...
	If ( !Empty(AllTrim(cFor)) .AND. !Empty(AllTrim(cLoja)) ) // VERIFICA SE ACHOU O FORNECEDOR
		...
		If ( !SE2->(DbSeek(xFilial("SE2") + cPrefixo + cNum + cParc + cTipo + cFor + cLoja )))
			...
			aAdd(aVetSE2, {"E2_DATALIB", dDataBase             ,               Nil})  // Lucas Raminelli - 11/04/2025 - precisa ja cair liberado o titulo
			aAdd(aVetSE2, {"E2_STATLIB", "03"             	   ,               Nil})  // Lucas Raminelli - 02/05/2025 - necessita colocar como 03 pra liberar
			aAdd(aVetSE2, {"E2_USUALIB", cUsername             ,               Nil})  // Lucas Raminelli - 02/05/2025 - necessita colocar o nome de quem liberou
